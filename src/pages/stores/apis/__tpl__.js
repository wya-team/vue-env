const api = {
	/**
	 * 测试用例
	 */
	'TPL_MAIN_GET': '/test.json',
	'TPL_SECOND_GET': '/test.json',
	'TPL_THIRD_ONE_GET': '/test.json',
	'TPL_THIRD_TWO_GET': '/test.json',
	'TPL_THIRD_THREE_GET': '/test.json',
	'TPL_TABLE_INFO_GET': '/test.json',
	'TPL_TABLE_LIST_GET': '/test.json',
	'TPL_TABLE_ITEM_DELETE': '/test.json',
	'TPL_TABLE_ITEM_POST': '/test.json',
	'TPL_TABLE_ITEM_PUT': '/test.json',
	'TPL_LIST_GET': '/list.json',
	'TPL_TABLE1_LIST_GET': '/list.json',
	'TPL_TABLE2_LIST_GET': '/list.json',
	'TPL_TABLE3_LIST_GET': '/list.json',
	'TPL_TABLE3_TABS_GET': '/tabs.json',
};
export default api;
